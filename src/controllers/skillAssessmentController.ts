...
// Logika skill gap — sesuai PDF
const calculateSkillGap = (answers: {
  score?: number | null;
  ...
  const skillComparison: Record<string, { yourScore: number; targetScore: number; status: string }> = {};

  let totalScore = 0;
  let totalCorrect = 0;
  let totalIncorrect = 0;

  answers.forEach(({ score, isCorrect, skillName, difficulty, questionType }) => {
    if (questionType === 'rating' && score != null) {
      totalScore += score;

      // Level berdasarkan skor
      let level: string;
      if (score <= 2) level = 'basic';
  ...
  return { overallScore, matchedSkills, missingSkills, skillLevels, skillComparison, totalCorrect, totalIncorrect };
};
  ...
  try {
    ...
    res.json({
      ...
      data: questions.map(q => ({
        ...
        scenario: q.scenario,
        hint: q.hint,
        // Jangan expose correctAnswer ke frontend!
      })),
    });
  } catch {
...
export const submitAssessment = async (req: authRequest, res: Response): Promise<void> => {
  try {
    ...
    // Gabungkan jawaban dengan data pertanyaan + cek essay correctness
    const answersWithMeta = answers.map((answer) => {
      ...
      if (question.questionType === 'essay' && answer.essayAnswer && question.correctAnswer) {
        // Simple keyword matching — bisa dikembangkan dengan AI nanti
        const keywords = question.correctAnswer.toLowerCase().split(' ').filter(w => w.length > 4);
    ...
    const { overallScore, matchedSkills, missingSkills, skillLevels, skillComparison, totalCorrect, totalIncorrect } =
    ...
    // Response sesuai PDF — termasuk answer review
    res.status(201).json({
